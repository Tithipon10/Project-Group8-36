    ...
    <div class="col">
      <div class="card shadow">
        <div class="card-header border-0">
          <h2 class="mb-0">Edit Popular Product (แก้ไขสินค้ายอดนิยม)</h2>
        </div>
          ...
          <table class="table align-items-center table-flush ">
            <thead class="thead-light">
              <tr>
                ...
                <th scope="col" class="col-sm-6 text-center">แก้ไขสินค้ายอดนิยม</th>
            ...
            <tbody>
              <form action="{{url('/admin/popular/Update/'.$Edit_popular->id_popular)}}" method="post" enctype="multipart/form-data">
                  @csrf
                  <th scope="row" style="display: block; margin-left: auto; margin-right: auto;">
                      <img src="{{ asset('admin/Popular_images/'.$Edit_popular->image)}}" alt="" id="showImage" 
                  ...
                  <th scope="col">
                      {{-- <div class="form-group">
                          ...
                          <input type="text" class="form-control" name="type_product"
                              id="exampleFormControlInput1" value="{{$Edit_popular->type_product}}">
                          @error('type_product')
                              <span class="text-danger">{{ $message }}</span>
                          @enderror
                      </div> --}}
                      <div class="form-group">
                        ...
                        <select class="form-control" id="exampleFormControlSelect1" name="type_product">
                          <option value="{{$Edit_popular->type_product}}">{{ $Edit_popular->category->type_product }}</option>
                      ...
                      <div class="form-group">
                          ...
                          <input type="text" class="form-control" name="popular"
                              id="exampleFormControlInput1" value="{{$Edit_popular->popular}}">
                          @error('popular')
                              <span class="text-danger">{{ $message }}</span>
                          @enderror
                      </div>
                      <div class="form-group">
                          ...
                          <input type="text" class="form-control" name="price"
                              id="exampleFormControlInput1" value="{{$Edit_popular->price}}">
                          @error('price')
                              <span class="text-danger">{{ $message }}</span>
                          @enderror
                      </div>
